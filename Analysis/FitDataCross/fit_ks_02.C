    ...
    // vacuum polorization function of leptons
{
    ...
    double mb=4.18;
    double mt=173;
    //return (vpfl(me,s)+vpfl(mu,s)+vpfl(mtau,s)+vpf_had);
...
double rescs(double *ecm0,double *par)
{
    ...
    //double delta= 0.75*beta+beta*beta/24.0*(1.0/3*log(ecm0[0]/me/me)+2.0*pi*pi-37.0/4);
    ...
    //double qed=4.*pi*alpha2/(3.*s)*(1.+beta/2.*(2.*log(Xf)-log(1.-Xf)+1.5-Xf)+alpha/pi*(pi2/3.-0.5))*vacc(ecm0[0]);
    //double qed=4.*pi*alpha2/(3.*s)*( (1+delta)*(pow(Xf,beta)-beta*log(1-Xf))+(-beta-beta*beta/4.0)*Xf+Xf*Xf/2*(beta/2-3.0/8.0*beta*beta) ); //tested
    //double qed=4.*pi*alpha2/(3.*s)*( beta*(1+delta)*(pow(Xf,beta) + log(1-Xf))*(beta*beta*5.0/8.0-beta*(0.5+delta))-(beta/2.0-3.0/8.*beta*beta)*Xf ); //tested far from true
    //double qed1=4.*pi*alpha2/(3.*s)*( beta*(1+delta)*(pow(Xf,beta)/beta-log(1-Xf)) );
    //double qed2=4.*pi*alpha2/(3.*s)*( (beta+beta*beta/4)*(pow(Xf,beta+1)/(beta+1)-(Xf-1+log(fabs(Xf-1)))) );
    //double qed3=4.*pi*alpha2/(3.*s)*( (beta/2-3./8.*beta*beta)*(pow(Xf,beta+2)/(beta+2)-0.5*((Xf-1)*(Xf-1)+4*(Xf-1)+2.*log(fabs(Xf-1)))) );
    //double qed = qed1+qed2+qed3;
    ...
    // add by zheng bo
    //double ampsim_p1 = 12*pi*GE*GT*BfKsX/s/s;
    //double ampsim_p21= (1+delta)*( pow(a,beta-2))*phi(scos,beta);
    //double ampsim_p22= (-beta+0.75*pow(beta,2))*pow(a,beta-1)*phi(scos,beta+1);
    //double inter=ampsim_p1*(ampsim_p21+ampsim_p22);
...
//double qedxs(double W)
{
    //double mmu=105.658e-3;
    //double a=mmu*mmu/s;
    //double b=1.0-4*a;
    double COEF=4.0*pi*alpha*alpha*389379.6623/3.0/s;//*sqrt(b)*(1.0+2.0*a);
    //double COEF = 86.8/s;
    return COEF;
    //for(int i=0; i<Nqed; i++){
    //    if(W>=Eqed[i]&&W<Eqed[i+1]) {
    //        if(Eqed[i]!=Eqed[i+1]) return CSqed[i]+(W-Eqed[i])*(CSqed[i+1]-CSqed[i])/(Eqed[i+1]-Eqed[i]);
    //        else {
    //            cerr<<"Error: x1=x2,this is not permitted"<<endl;
    //            exit(-1);
    //        }
...
double funs(double *x, double *par)
{
    ...
    //dXS = dXS + TMath::Power(epsilon,beta)*qedxs(par[0]*sqrt(1.0-x[0]));
    return dXS;
}
...
double intXS(double *ecm, double *par)
{
    ...
    //fXS->SetParameter(7, par[6]); //Form factor
    //fXS->SetParameter(7, par[7]); //GF
    double my_intXS=fXS->Integral(TMath::Min(ecm[0],par[0])-20*res, TMath::Max(ecm[0],par[0])+20*res);
    //double my_intXS=fXS->Integral(TMath::Min(ecm[0],par[0])-20*res, TMath::Max(ecm[0],par[0])+20*res)*pow(par[6]/pow(ecm[0],2),2); //*pow(par[6]/pow(ecm[0],2),2) is the form factor for corespoinding channel
    ...
    //double my_intXS_qed = hs*( my_intXS_qed2)*(1.0+delta) * vacc(ecm[0]); //
    //double my_intXS_qed = ( (my_intXS_qed1 + my_intXS_qed2)*(1.0+delta) + my_intXS_qed3 )* vacc(ecm[0])*pow(par[6]/pow(ecm[0],2),2);
    //double my_intXS_qed = ( (my_intXS_qed1 + my_intXS_qed2)*(1.0+delta) + my_intXS_qed3 );
    //double my_intXS_qed = ( (my_intXS_qed1 + my_intXS_qed2) + my_intXS_qed3 )* vacc(ecm[0]);
    //double my_intXS_qed = (my_intXS_qed2 )*(1.0+delta);
...
int main()
{
    ...
    //std::cout<<"CLHEP::fine_structure_const = "<<double(CLHEP::fine_structure_const)<<std::endl;
    //std::cout<<"CLHEP::pi = "<<double(CLHEP::pi)<<std::endl;
    //std::cout<<"CLHEP::electron_mass_c2 = "<<CLHEP::electron_mass_c2<<std::endl;
    //std::cout<<"CLHEP::pi2 = "<<CLHEP::pi2<<std::endl;
    //ifstream vacfile("../fit_moxh/vacc.log");//,ios::in);
    //ifstream qedfile("qedxs.log");//,ios::in);
    //for(int i=0;i<Nqed;i++)
    //{
    //    qedfile>>Eqed[i]>>CSqed[i]>>eCSqed[i];
    //    Eqed[i]=Eqed[i]*2.0;
    //}
    ...
    ifstream cross("cross_08_ks.txt");
    for (int i=0;i<Npoints;i++){
         cross >> expcs_cor[i]>>expcserr_cor[i];
    }
...
//    TLegend *leg = new TLegend(0.15, 0.75, 0.4, 0.90);
//    leg->AddEntry(grapherrors_cor, "J/#psi#rightarrow K_{S}X", "PL");
//    leg->AddEntry(thecs, "fit", "PL");
//    leg->SetFillColor(0);
//    leg->SetBorderSize(0);
//    leg->Draw();


    canvas->SaveAs("fit_ks_02.root");
    return 0;
}
