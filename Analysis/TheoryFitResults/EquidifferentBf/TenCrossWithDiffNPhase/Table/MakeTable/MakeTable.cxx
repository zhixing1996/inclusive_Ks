...
void MakeTable(){
     ...
     table<<"Specific combination can be seen in: ../../../../../TheoryFit/EquidifferentBf/TenCrossWithDiffNPhase/GenCombination/combination.txt with branch ratio rank stated above"<<endl;
     ...
     for (Int_t i=0;i<11;i++){
         ...
         for (Int_t j=0;j<tot;j++){
               if (Count[j]==i)
                  table<<j<<"\t"<<"0.325"<<"\t"<<phase[j]<<"+/-"<<phaseerr[j]<<"\t"<<bf[j]<<"+/-"<<bferr[j]<<"\t"<<(bf[j]-0.325)/0.325*100<<endl;
         }
     }
