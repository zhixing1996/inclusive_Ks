...
int main(int argc, char *argv[])
{
   ...
   for (int i=0;i<Npoints;i++ ){
    ...
    ofstream paras;
    TString orange="/besfs/groups/tauqcd/jingmq/inclusive_Ks/EquidifferentBf/TwoCrossWithDiffPhase/logfile/fit_ks_phase";
    for (int i=1; i<argc;i++){
      orange+="_";
      orange+=argv[i];
     }
    ...
    stream1 << argv[1];
    TString ap="../0.01/"+stream1.str()+"/fit_ks_phase.txt";
    ifstream cross1(ap);
    for(int i=0;i<Npoints;i++){
   ...
   for (int i=0;i<Npoints;i++ ){
    ...
    TString banana="/besfs/groups/tauqcd/jingmq/inclusive_Ks/EquidifferentBf/TwoCrossWithDiffPhase/cross/fit_ks_phase";
    for (int i=1; i<argc;i++){
      banana+="_";
      banana+=argv[i];
     }
