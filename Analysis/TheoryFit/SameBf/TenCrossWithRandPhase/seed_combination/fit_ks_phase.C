...
int main(int argc, char *argv[])
{
   ...
   for (int i=0;i<Npoints;i++ ){
    ...
    TString apple="/besfs/groups/tauqcd/jingmq/inclusive_Ks/SameBf/TenCrossWithRandPhase/rootfile/fit_ks_phase_combination";
    for (int i=1; i<argc;i++){
      apple+="_";
      apple+=argv[i];
     }
    ...
    ofstream paras;
    TString orange="/besfs/groups/tauqcd/jingmq/inclusive_Ks/SameBf/TenCrossWithRandPhase/logfile/fit_ks_phase_combination";
    for (int i=1; i<argc;i++){
      orange+="_";
      orange+=argv[i];
     }
    ...
    int Phase[10];
    ifstream PHASE("../Phase_combination.txt");
    for(int i=0;i<10;i++){
        PHASE >> Phase[i];
    }

    double Bf=0, Bf_test;
    ifstream BF("../Bf_combination.txt");
    for(int i=0;i<10;i++){
        BF >> Bf_test;
        Bf+=Bf_test;
    }
   ...
   for (int i=0;i<Npoints;i++ ){
    ...
    TString banana="/besfs/groups/tauqcd/jingmq/inclusive_Ks/SameBf/TenCrossWithRandPhase/cross/fit_ks_phase_combination";
    for (int i=1; i<argc;i++){
      banana+="_";
      banana+=argv[i];
     }
