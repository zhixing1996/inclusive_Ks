...
int main(int argc, char *argv[])
{
   ...
   for (int i=0;i<Npoints;i++ ){
    ...
    if(tag10==1){
      ifstream ten("../10/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          ten >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }

    if(tag20==1){
      ifstream twenty("../20/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          twenty >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }

    if(tag30==1){
       ifstream thirty("../30/fit_ks_phase.txt");
       for(int i=0;i<Npoints;i++){
           thirty >> ecm0[i] >> temp_cross;
           expcs_cor[i]+= temp_cross;
       }
    }

    if(tag40==1){
      ifstream forty("../40/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          forty >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      } 
    }

    if(tag50==1){
      ifstream fifty("../50/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          fifty >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }

    if(tag60==1){
      ifstream sixty("../60/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          sixty >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }

    if(tag70==1){
      ifstream seventy("../70/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          seventy >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }
  
    if(tag80==1){
      ifstream eighty("../80/fit_ks_phase.txt");
      for(int i=0;i<Npoints;i++){
          eighty >> ecm0[i] >> temp_cross;
          expcs_cor[i]+= temp_cross;
      }
    }
